...
4. **date系オブジェクトエラーの修正** - インデックス化時のdate型変換エラー修正（969件の未インデックスファイル対応）
5. **検索機能の拡張** - タグ検索、キーワード検索、日付範囲検索、文字数範囲検索、類似ドキュメント検索の実装
6. **定期ジョブスクリプト** - Git同期とデイリーレポート生成
7. **Vault指定の柔軟化** - GitHubリポジトリ設定の実地確認
8. **テスト拡充** - 統合テストとカバレッジ向上
...
**【絶対遵守】Obsidianファイル編集の鉄則**

本プロジェクトでは、類似リンク挿入・タグ自動挿入などの機能がObsidianのMarkdownファイルを自動編集します。
以下のルールは**絶対に守られなければなりません**。

### 📌 AI自動生成セクションの定義

各Obsidianファイルには、以下のマーカーで囲まれた「AI自動生成セクション」が存在する場合があります：
...
1. **AI自動生成セクション以外の編集は絶対に禁止**
   - このセクション外のテキストは**一切変更してはならない**
   - 既存の本文、見出し、リンク、タグ、フロントマターなど、全てを保護する

2. **セクションが存在しない場合の対応**
   - ファイル末尾に新規追加することのみ許可される
   - 既存コンテンツには一切触れない

3. **セクション検出の厳密性**
   - `========== AI AUTO-GENERATED SECTION START ==========`で開始
   - `========== AI AUTO-GENERATED SECTION END ==========`で終了
   - このマーカーの完全一致のみを認識し、曖昧なパターンマッチは行わない
...
- `app/core/document_updater.py` がセクション管理の責任を持つ
- `extract_ai_section()` メソッドで厳密にセクションを抽出
- `update_document()` メソッドで安全な更新を保証
- 除外フォルダ（`excluded_folders`）設定でファイルを保護

### 📁 除外フォルダ設定

以下のフォルダ内のファイルは**自動編集対象外**です（`.env`で設定可能）：

- `01DIARY` - 個人的な日記
- `02TEMPLATES` - テンプレートフォルダ
- `06MOC` - Map of Contents（目次）
- `10KANBAN` - カンバンボード
- `11MEDIA` - メディアファイル
- `Excalidraw` - 図形ファイル
- `Maybe` - 一時的なメモ
- `Omnivore` - Omnivoreからのインポート
...
**ルートディレクトリのファイルも自動的に除外されます**（`EXCLUDE_ROOT_FILES=true`）

### 🔍 違反チェック

コードレビュー時に以下を確認：

1. `document_updater.py` 以外でファイル書き込みを行っていないか
2. セクション外のコンテンツに触れていないか
3. 除外フォルダの判定が適切に行われているか

**このルールに違反するコードは絶対にマージしない**

---

## 🎉 Phase 1-2完了: AI自動ファイル編集システム

**完了日**: 2025-01-15

### 実装完了した機能

#### 1. 設定管理の拡張 (`app/core/config.py`)
- ✅ 除外フォルダリスト（16フォルダ）
  - 01DIARY, 02TEMPLATES, 06MOC, 10KANBAN, 11MEDIA
  - Excalidraw, Maybe, Omnivore, model_cache, PythonScripts, github
  - .chroma_db, .claude, .devcontainer, .smtcmp_json_db, .smtcmp_vector_db
- ✅ ルートディレクトリファイル除外設定 (`exclude_root_files`)
- ✅ 自動リンク挿入のON/OFF設定
- ✅ 自動タグ挿入のON/OFF設定
- ✅ 類似度閾値・最大リンク数の設定

#### 2. セクション管理モジュール (`app/core/document_updater.py`)
- ✅ 厳密なセクション検出（偶然の一致を防止）
  - `========== AI AUTO-GENERATED SECTION START ==========`
  - `========== AI AUTO-GENERATED SECTION END ==========`
- ✅ セクション外コンテンツの完全保護
...
#### 3. 類似リンク挿入機能 (`app/core/link_inserter.py`)
- ✅ ChromaDBからの類似ドキュメント検索
- ✅ 重複ファイル検出（ファイル名ベース）
- ✅ 類似度フィルタリング
- ✅ バッチ処理対応
- ✅ 統計情報の収集

#### 4. タグ自動挿入機能 (`app/core/tag_inserter.py`)
- ✅ 10種類の主要例外パターン検出
  - YAMLフロントマター、コードブロック、インラインコード
  - Obsidianリンク、Markdownリンク、画像リンク
  - URL、既存タグ、HTMLコメント、見出し
- ✅ タグの正規化機能
- ✅ バッチ処理対応
- ✅ 類似リンクとの同時挿入対応

#### 5. ドキュメント整備
- ✅ `.env.example` - 全設定項目を網羅
- ✅ `README.md` - AI編集ルールの明確化
- ✅ `docs/TODO.md` - 詳細なルール記載

### 安全性の保証

🛡️ **3重の安全保護:**
1. **厳密なセクション検出** - 偶然の一致を完全防止
2. **除外フォルダ機能** - 16フォルダ + ルートディレクトリ
3. **例外パターン検出** - 10種類のパターンを回避
...
**目標**: 類似リンク・タグ挿入をデイリーバッチに組み込む

**タスク**:
- [ ] `scripts/daily_link_tag_update.py` の作成
  - [ ] 全Markdownファイルのスキャン
  - [ ] 除外フォルダ・ルートファイルのフィルタリング
  - [ ] ChromaDBから類似ドキュメント取得
  - [ ] LLMからタグ生成
  - [ ] バッチ処理での一括更新
  - [ ] 進捗表示（tqdm）
  - [ ] エラーハンドリングとログ出力
- [ ] `pyproject.toml` の `[project.scripts]` に `oc-daily-update` を追加
- [ ] systemdタイマーまたはcronへの登録
  - [ ] 毎日深夜2:00に実行
  - [ ] ログローテーション設定
- [ ] `.env` に実行間隔・バッチサイズの設定追加

**依存関係**:
- `app/core/link_inserter.py` (完了)
- `app/core/tag_inserter.py` (完了)
- `app/services/vector_db_service.py` (既存)
- `app/services/llm_service.py` (既存)
...
**目標**: `bulk_tag_inserter.py` の機能を統合

**タスク**:
- [ ] TF-IDF分析結果からタグを抽出
- [ ] 既存の `tag_inserter.py` と統合
- [ ] スコアベースのタグ選定
...
**目標**: `moc_generator.py` / `generate_moc_from_analysis.py` の機能を統合
...
1. **Phase 1.1 (FastAPIアプリ実装)** - 検索UIとAPI基盤
